...
class Runescape:
    ...
    async def make_url_profile(self, runescape_name, activities):
        return "https://apps.runescape.com/runemetrics/profile/profile?user={}&activities={}".format(runescape_name, activities)

    async def make_url_player_details(self, runescape_name):
        return "http://services.runescape.com/m=website-data/playerDetails.ws?names=%5B%22{}%22%5D&callback=jQuery000000000000000_0000000000&_=0".format(runescape_name)
    ...
    async def get_player_details(self, runescape_name):
        ...
        json_data = json.loads(data.replace("jQuery000000000000000_0000000000([", "").replace("]);", ""))
        return json_data
            # return await resp.json()

    async def get_data_profile(self, runescape_name, activities):
        async with self.session.get(await self.make_url_profile(runescape_name, activities)) as resp:
            return await resp.json()
    ...
    async def profile_embed(self, profile, details):
        ...
        em.set_thumbnail(url="http://secure.runescape.com/m=avatar-rs/{}/chat.png".format(profile.name.replace(" ", "%20")))
