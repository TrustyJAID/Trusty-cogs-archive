...
class Hockey:
    ...
    async def clear_broken_channels(self, ctx):
        for team in list(self.settings):
            remove_list = []
            for channel in self.settings[team]["channel"]:
                chn = self.bot.get_channel(id=channel)
                if chn is None:
                    remove_list.append(channel)
            for channel in remove_list:
                print(channel)
                self.settings[team]["channel"].remove(channel)
        dataIO.save_json("data/hockey/settings.json", self.settings)
    ...
    async def remove_goals(self, ctx, team, channel:discord.TextChannel=None):
        ...
        if channel is None:
            channel = ctx.message.channel
        if team not in self.settings:
            await ctx.message.channel.send( "I am not posting {} goals in {}".format(team, channel.mention))
            return
        if channel.id in self.settings[team]["channel"]:
