...
class Hockey:
    ...
    def __init__(self, bot):
        ...
        self.config = Config.get_conf(self, 133925647778)
    ...
    async def getemojiid(self, ctx):
        ...
        emojilist = []
        for team in self.config:
            self.config[team]["created_channel"] = []
        # for emoji in guild.emojis:
            # for team in self.teams:
                # if emoji.name[:5].replace(".", "") in team:
                    # emoji_id = emoji.name + ":" + emoji.id
                    # self.teams[team]["emoji"] = emoji_id
        dataIO.save_json("data/hockey/settings.json", self.config)
    ...
    async def team_playing(self, games):
        ...
        links = {}
        for game in games:
            if game["teams"]["away"]["team"]["name"] in self.config and game["status"]["abstractGameState"] != "Final":
                is_playing = True
                links[game["teams"]["away"]["team"]["name"]] = game["link"]
            if game["teams"]["home"]["team"]["name"] in self.config and game["status"]["abstractGameState"] != "Final":
                is_playing =True
    ...
    async def check_team_goals(self, goals, team, score_msg):
        ...
        goal_list = list(self.config[team]["goal_id"])
        for goal in goals:
            ...
            if goal_id not in goal_list:
                ...
                self.config[team]["goal_id"][goal_id] = {"goal":goal,"messages":msg_list}
                dataIO.save_json("data/hockey/settings.json", self.config)
            if goal_id in goal_list:
                # Checks if the goal data has changed and edits all previous posts with new data
                old_goal = self.config[team]["goal_id"][goal_id]["goal"]
                if goal != old_goal:
                    print("attempting to edit")
                    old_msgs = self.config[team]["goal_id"][goal_id]["messages"]
                    self.config[team]["goal_id"][goal_id]["goal"] = goal
                    dataIO.save_json("data/hockey/settings.json", self.config)
                    await self.post_team_goal(goal, team, score_msg, old_msgs)

        for old_goal in goal_list:
            if old_goal not in goal_ids:
                old_msgs = self.config[team]["goal_id"][old_goal]["messages"].items()
                for channel_id, message_id in old_msgs:
                    ...
                    try:
                        ...
                        print("I can't delete messages in {}".format(channel.guild.name))
                        pass
                del self.config[team]["goal_id"][old_goal]
                dataIO.save_json("data/hockey.settings.json", self.config)
        ...
        if og_msg is None:
            if "oilers" in goal["team"]["name"].lower():
                try:
                    ...
                    pass
            for channels in self.config[team]["channel"]:
                role = None
    ...
    async def hockeytwitter(self, ctx):
        guild = self.bot.get_guild(id="381567805495181344")
        for team in self.teams:
            team = team.replace(".", "")
            team = team.replace(" ", "-")
            if team.startswith("Montr"):
                team = "montreal-canadiens"
            await self.bot.create_channel(guild, name=team.lower() + "-twitter")
    ...
    async def add_goals(self, ctx, team, channel:discord.Channel=None):
        ...
        if team not in self.config:
            self.config[team] = {"channel":[channel.id],"created_channel":[], "goal_id": {}}
        if channel.id in self.config[team]["channel"]:
            await self.bot.send_message(ctx.message.channel, "I am already posting {} goals in {}!".format(team, channel.mention))
            return
        self.config[team]["channel"].append(channel.id)
        dataIO.save_json("data/hockey/settings.json", self.config)
    ...
    async def remove_goals(self, ctx, team, channel:discord.Channel=None):
        ...
        if team not in self.config:
            ...
            return
        if channel.id in self.config[team]["channel"]:
            self.config[team]["channel"].remove(channel.id)
            dataIO.save_json("data/hockey/settings.json", self.config)
    ...
    async def team_role(self, ctx, *, team):
        ...
        if guild.id != "381567805495181344":
            await self.bot.send_message(ctx.message.channel, "Sorry that only works on TrustyJAID's Oilers guild!")
            return
        try:
            team = [team_name for team_name in self.teams if team.lower() in team_name.lower()][0]
        except IndexError:
            await self.bot.say("{} is not an available team!".format(team))
            return
        role = [role for role in guild.roles if role.name == team][0]
