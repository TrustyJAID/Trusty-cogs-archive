...
class Gabai(getattr(commands, "Cog", object)):
    ...
    def __init__(self, bot):
        ...
        default_global = {"api_token":{"client_id":"", "client_secret":"", "token":{}, "refresh_time":0}}
    ...
    async def gab_menu(self, ctx:commands.Context, post_list: list,
    ...
    async def feed(self, ctx, username:str, before_date:str=None):
        ...
        if before_date is None:
            ...
            before_date = datetime.strptime(before_date, "%d-%m-%Y").strftime("%Y-%m-%dT%H:%M:%S+00:00")
        try:
            feed_info = await self.get_gab_response(f"users/{username}/feed/", {"before":before_date})
    ...
    async def token(self, ctx, client_id:str, client_secret:str):
        ...
        await ctx.send("Please go to the following url and provide the code supplied: {}".format(url))
