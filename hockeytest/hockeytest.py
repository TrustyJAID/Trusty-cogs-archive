...
class Hockeytest:
    ...
    async def get_team_goals(self):
        ...
        while self is self.bot.get_cog("Hockeytest"):
            ...
            # games = ["https://statsapi.web.nhl.com/api/v1/game/2018010061/feed/live"]
            games_playing = False
            while games != []:
                to_remove = []
                games_playing = True
    ...
    @commands.group(pass_context=True, name="hockeytest", aliases=["ht"])
    async def hockey_test(self, ctx):
    ...
    @hockey_test.command(hidden=True)
    @checks.is_owner()
    ...
    @hockey_test.command(name="poststandings", aliases=["poststanding"])
    ...
    @hockey_test.command(pass_context=True, name="add", aliases=["add_goals"])
    @checks.admin_or_permissions(manage_channels=True)
    async def add_goals(self, ctx, team, channel:discord.TextChannel=None):
        ...
        if team_name in cur_teams:
            await ctx.send("I'm already posting goals from {}".format(team_name))
            return
        if "all" in cur_teams and team_name != "all":
            cur_teams.remove("all")
        if team_name == "all":
            cur_teams = []
        if not channel.permissions_for(guild.me).embed_links:
    ...
    @hockey_test.command(pass_context=True, name="del", aliases=["remove", "rem"])
    @checks.admin_or_permissions(manage_channels=True)
    ...
    @hockey_test.command(pass_context=True, name="role")
    ...
    @hockey_test.command(pass_context=True, name="goals")
    ...
    @hockey_test.command(pass_context=True)
    ...
    @hockey_test.command(pass_context=True, aliases=["score"])
